...
睁大眼睛盯一眼博客右侧：没错！恭喜你看到了五个竖排的小方块——回到顶部、回首页、随机一篇、目录、跳转评论区。

但是Fuwari 原版这里只有一个「回到顶部」按钮，桌面大屏的目录又固定在右边缘（2xl 以上才显示），窄屏幕直接没目录可用。我又是如何实现这些功能扩展的呢？欲知后事如何，请看下文分解🐶
...
注意，由于全站开了 Swup，切页有极小概率不刷新，这是工具栏最容易失控的地方，处理就三条：
